"""
Menu system constants and configuration.

This module contains all the parameter groupings, hints, and configuration
constants used by the overlay menu in the simulation app.
"""
...
HUD_METRICS_INTERVAL_S = 0.5
GRAD_STAT_ALPHA = 0.08
GRAD_CELL_DIV = 20.0

# Base colors for particle populations
POS_BASE_COLOR = (188, 214, 255, 220)  # Blue-ish for M+
NEG_BASE_COLOR = (255, 184, 107, 220)  # Orange-ish for M-

# Gradient color stops for M+ visualization
GRADIENT_STOPS = [
    (0.0, (40, 100, 190)),    # Cold: blue
    (0.45, (245, 245, 245)),  # Neutral: white
    (0.75, (255, 170, 90)),   # Warm: orange
    (1.0, (230, 60, 50)),     # Hot: red
]

# Labels for gradient modes
GRADIENT_LABELS = {
    "mix": "M+ mix",
    "speed": "M+ speed",
    "force": "M+ force",
...
# Parameters that require simulation reset when changed
RESET_KEYS = {
    "init_mode",
    "particle_count",
    "positive_count",
...
# Janus force parameters (depend on janus_enabled)
JANUS_DEP_KEYS = {
    "janus_g",
    "force_backend",
    "force_tile_size",
...
# Maps parameter keys to (main_group, sub_group) for menu organization
MENU_GROUPS = {
    # Init
...
PARAM_HINTS = {
    ...
    "population_mode": "Count mode: total or explicit.",
...
def get_menu_group(key: str) -> tuple[str | None, str | None]:
    """
    Get the menu group for a parameter key.
    
    Args:
        key: Parameter name
    
    Returns:
        (main_group, sub_group) tuple, or (None, None) if not found
    """
...
def get_param_hint(key: str) -> str | None:
    """
    Get the hint/tooltip for a parameter.
...
def is_reset_required(key: str) -> bool:
    """
    Check if changing a parameter requires simulation reset.
